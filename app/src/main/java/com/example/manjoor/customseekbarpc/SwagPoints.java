...
public class SwagPoints extends View {
    ...
    /**
     * The maximum value that {@link SwagPoints } can be set.
     */
    ...
    public void setProgressDisplayAndInvalidate(int progressDisplay) {
        ...
        if(mOnSeekBarChangeListener != null) {
            mOnSeekBarChangeListener.onPointsChanged(this, mProgressDisplay, true);
        }
        invalidate();
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SwagPoints, 0, 0);
            Drawable indicator = typedArray.getDrawable(R.styleable.SwagPoints_csb_thumbDrawable);
            if (indicator != null) mThumbDrawable = indicator;

            mProgressDisplay = typedArray.getInteger(R.styleable.SwagPoints_csb_progress, mProgressDisplay);
            mThumbSize = typedArray.getDimensionPixelSize(R.styleable.SwagPoints_csb_thumbSize, 50);

            mMin = typedArray.getInteger(R.styleable.SwagPoints_csb_min, mMin);
            mMax = typedArray.getInteger(R.styleable.SwagPoints_csb_max, mMax);
            mStep = typedArray.getInteger(R.styleable.SwagPoints_csb_step, mStep);


            mTextSize = (int) typedArray.getDimension(R.styleable.SwagPoints_csb_textSize, mTextSize);
            textColor = typedArray.getColor(R.styleable.SwagPoints_csb_textColor, textColor);
            mIsShowText = typedArray.getBoolean(R.styleable.SwagPoints_csb_isShowText, mIsShowText);

            mProgressWidth = (int) typedArray.getDimension(R.styleable.SwagPoints_csb_progressWidth, mProgressWidth);
            progressColor = typedArray.getColor(R.styleable.SwagPoints_csb_progressColor, progressColor);

            mArcWidth = (int) typedArray.getDimension(R.styleable.SwagPoints_csb_arcWidth, mArcWidth);
            arcColor = typedArray.getColor(R.styleable.SwagPoints_csb_arcColor, arcColor);
    ...
    public interface OnSeekBarChangedListener {
         ...
         *
         * @param swagPoints The CircleSeekBar view whose value has changed
